...
const TAG_CONFIG: Record<string, { label: string; icon: React.ReactNode; color: string; bg: string }> = {
  friend: { label: "Friend", icon: <Users className="size-3" />, color: "text-success", bg: "bg-success/10" },
  enemy: { label: "Enemy", icon: <Crosshair className="size-3" />, color: "text-destructive", bg: "bg-destructive/10" },
  trader: { label: "Trader", icon: <ShoppingCart className="size-3" />, color: "text-chart-2", bg: "bg-chart-2/10" },
  guild_mate: { label: "Guild", icon: <Shield className="size-3" />, color: "text-primary", bg: "bg-primary/10" },
  other: { label: "Other", icon: <Eye className="size-3" />, color: "text-muted-foreground", bg: "bg-muted/10" },
};
...
export function WatchlistPage() {
  ...
  const filtered = filter === "all" ? watchlist : watchlist.filter((w) => w.tag === filter);

  const sorted = [...filtered].sort((a, b) => {
    const mult = sortAsc ? 1 : -1;
    if (sortBy === "level") return (b.watchedPlayerLevel - a.watchedPlayerLevel) * mult;
    if (sortBy === "str") return ((b.watchedPlayerStr || 0) - (a.watchedPlayerStr || 0)) * mult;
    if (sortBy === "lastActivity") return ((a.watchedPlayerLastActivity || 0) - (b.watchedPlayerLastActivity || 0)) * mult;
    ...
    <div className="p-3 md:p-4 space-y-3 max-w-4xl">
      ...
      <div className="flex gap-1 bg-secondary/50 rounded-lg p-0.5">
        {(["all", "friend", "enemy", "trader"] as const).map((f) => {
          const count = f === "all" ? watchlist.length : watchlist.filter((w) => w.tag === f).length;
          return (
            <button
              key={f}
              type="button"
            ...
            >
              {f === "all" ? `All (${count})` : `${f.charAt(0).toUpperCase() + f.slice(1)}s (${count})`}
            </button>
          );
        })}
      </div>
      ...
      <div className="flex items-center gap-1.5 text-[10px]">
        ...
        {(["level", "str", "lastActivity", "name"] as const).map((key) => (
          ...
          >
            {key === "level" ? "Level" : key === "str" ? "STR" : key === "lastActivity" ? "Activity" : "Name"}
        ...
        <div className="space-y-1.5">
          {sorted.map((player) => {
            ...
            const hpPct = player.watchedPlayerMaxHp
              ? Math.round(((player.watchedPlayerHp || 0) / player.watchedPlayerMaxHp) * 100)
              : null;
            const now = Date.now();
            ...
            return (
              <div key={player._id} className={`game-card ${
                player.tag === "enemy" ? "border-destructive/15" :
                player.tag === "friend" ? "border-success/15" : ""
              }`}>
                <div className="flex items-start gap-3">
                  {/* Tag indicator */}
                  <div className={`size-9 rounded-lg flex items-center justify-center shrink-0 ${tagConfig.bg}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    {/* Name + tag + online */}
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold truncate">{player.watchedPlayerName}</span>
                      <span className={`text-[9px] px-1.5 py-0.5 rounded ${tagConfig.bg} ${tagConfig.color} uppercase tracking-wider font-medium`}>
                        {tagConfig.label}
                      </span>
                      {player.watchedPlayerSafeMode && (
                        <span className="text-[9px] px-1 py-0.5 rounded bg-success/15 text-success">SAFE</span>
                      )}
                    ...
                    <div className="flex items-center gap-3 mt-0.5 text-[10px] text-muted-foreground flex-wrap">
                      ...
                      {player.watchedPlayerGuildName && (
                        <span className="text-primary/70">⚜ {player.watchedPlayerGuildName}</span>
                      )}
                      <span className="flex items-center gap-0.5">
                        <Clock className="size-2.5" /> {activityLabel}
                      </span>
                      {player.watchedPlayerGold !== undefined && player.watchedPlayerGold > 0 && (
                        <span className="text-gold-accent">{formatGold(player.watchedPlayerGold)}</span>
                      )}
                    </div>

                    {/* Combat Stats */}
                    {(player.watchedPlayerStr || player.watchedPlayerDef || player.watchedPlayerDex) && (
                      <div className="flex items-center gap-3 mt-1 text-[10px]">
                        {player.watchedPlayerStr !== undefined && (
                          <span className="flex items-center gap-0.5 text-chart-4">
                            <Swords className="size-2.5" /> {formatNumber(player.watchedPlayerStr)}
                          </span>
                        )}
                        {player.watchedPlayerDef !== undefined && (
                          <span className="flex items-center gap-0.5 text-chart-1">
                            <Shield className="size-2.5" /> {formatNumber(player.watchedPlayerDef)}
                          </span>
                        )}
                        {player.watchedPlayerDex !== undefined && (
                          <span className="flex items-center gap-0.5 text-chart-5">
                            <Zap className="size-2.5" /> {formatNumber(player.watchedPlayerDex)}
                          </span>
                        )}
                      </div>
                    )}
                    ...
                    {/* Notes */}
                    {player.notes && (
                      <p className="text-[10px] text-muted-foreground/60 mt-1 italic">📝 {player.notes}</p>
                    )}

                    {stale && (
                      <span className="text-[9px] text-warning mt-0.5 inline-block">
                        ⚠ Data may be stale — last checked {Math.floor(lastCheckedAge / 3600000)}h ago
                      </span>
                    )}
                  </div>
      ...
      <p className="text-[10px] text-muted-foreground text-center">
        Mock data · Player stats update with API refresh · {watchlist.length} players tracked
      </p>
    </div>
  );
}
