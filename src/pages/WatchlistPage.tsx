      ...
      <div className="flex items-center gap-1.5 text-[10px]">
        ...
        {(["level", "str", "lastActivity", "name"] as const).map(key => (
          <button
            ...
            aria-label={
              sortBy === key
                ? `Sort by ${key === "level" ? "Level" : key === "str" ? "STR" : key === "lastActivity" ? "Activity" : "Name"} ${sortAsc ? "ascending" : "descending"}`
                : `Sort by ${key === "level" ? "Level" : key === "str" ? "STR" : key === "lastActivity" ? "Activity" : "Name"}`
            }
            onClick={() => {
