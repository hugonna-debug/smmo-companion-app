...
export function DashboardPage() {
  ...
  const expPercent =
    player.expToNextLevel > 0
      ? Math.round(
          ((player.expToNextLevel - (player.expToNextLevel - player.exp)) /
  ...
  const kdRatio =
    player.pvpDeaths > 0
      ? (player.pvpKills / player.pvpDeaths).toFixed(2)
      : "∞";
  const energyPct =
    player.energy !== undefined && player.maxEnergy
      ? Math.round((player.energy / player.maxEnergy) * 100)
      : null;
  const qpPct =
    player.questPoints !== undefined && player.maxQuestPoints
      ? Math.round((player.questPoints / player.maxQuestPoints) * 100)
      : null;
  ...
  return (
    <div className="p-3 md:p-4 space-y-3 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {player.membership === 1 && (
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-rarity-celestial/15 text-rarity-celestial font-medium">
                VIP
              </span>
            )}
          </div>
      ...
      <div className="game-card space-y-2.5">
        ...
        <div>
          ...
          <div className="stat-bar">
            <div
              className="stat-bar-fill bg-destructive"
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[10px] font-mono text-blue-500">
                    {formatNumber(player.questPoints!)}/
                    {formatNumber(player.maxQuestPoints!)}
                  </span>
                </div>
                <div className="stat-bar">
                  <div
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {player.safeMode && <SafeModeWidget />}
        {dailyTotal > 0 && (
      ...
      <div className="grid grid-cols-3 gap-2">
        <StatCard
          ...
          label="Gold"
          value={formatGold(player.gold)}
        />
        <StatCard
          icon={<Landmark className="size-3.5 text-gold-accent" />}
          label="Bank"
          value={formatGold(player.bank ?? 0)}
        />
        <StatCard
          icon={<Gem className="size-3.5 text-info" />}
          label="Diamonds"
          value={formatNumber(player.diamonds ?? 0)}
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <StatCard
          icon={<Footprints className="size-3.5 text-chart-5" />}
          label="Steps"
          value={formatNumber(player.steps)}
        />
        <StatCard
          icon={<Swords className="size-3.5 text-chart-4" />}
          label="PvP Kills"
          value={formatNumber(player.pvpKills)}
        />
        <StatCard
          icon={<Skull className="size-3.5 text-muted-foreground" />}
          label="NPC Kills"
          value={formatNumber(player.npcKills)}
        />
      </div>

      {/* Tasks Summary Widget */}
      {tasks && tasks.length > 0 && <TasksSummaryWidget tasks={tasks} />}
          ...
          <div className="game-card hover:bg-secondary/40 transition-colors">
            ...
            <div className="grid grid-cols-2 gap-2">
              {modifiers.map(mod => {
                ...
                const typeLabel =
                  mod.modifierType === "step_speed"
                    ? "Speed"
                    : mod.modifierType === "experience"
                      ? "EXP"
                      : mod.modifierType === "drop_rate"
                        ? "Drop"
                        : mod.modifierType;
                return (
                  <div
        ...
        <div className="game-card">
          ...
          <div className="grid grid-cols-3 gap-2">
            <StatBreakdownV2
              label="STR"
              color="text-chart-4"
              total={player.totalStr ?? 0}
              rank={player.strRank}
            />
            <StatBreakdownV2
              label="DEF"
              color="text-chart-1"
              total={player.totalDef ?? 0}
              rank={player.defRank}
            />
            <StatBreakdownV2
              label="DEX"
              color="text-chart-5"
              total={player.totalDex ?? 0}
              rank={player.dexRank}
            />
          </div>
        </div>
      ...
      {skills.length > 0 && (
        <div className="game-card">
          <h2 className="text-xs font-semibold text-gold-dim uppercase tracking-wider mb-2">
            Skills
          </h2>
          <div className="grid grid-cols-2 gap-2">
            {skills.map(s => {
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[11px] font-semibold text-destructive">
                    {lowBosses.length} boss{lowBosses.length > 1 ? "es" : ""}{" "}
                    near death!
                  </p>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[11px] font-bold text-gold-accent">
                      {formatGold(pricePerDiamond)}
                    </p>
          ...
          {equipment.slice(0, 6).map(eq => (
            <div
              ...
              className={`flex items-center gap-2 p-1.5 rounded-md border ${
                eq.itemName
                  ? "border-blue-500/30 bg-secondary/30"
            ...
            >
              ...
              {eq.itemName ? (
                ...
                <span className="text-[11px] text-muted-foreground italic">
                  Empty {SLOT_LABELS[eq.slot] || eq.slot}
                </span>
              )}
            </div>
          ))}
      ...
      <div className="game-card">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px]">
          ...
          <StatRow
            label="Boss Kills"
            value={formatNumber(player.bossKills ?? 0)}
          />
          <StatRow
            label="Quests Performed"
            value={formatNumber(player.questsPerformed ?? 0)}
          />
          <StatRow
            label="Quests Completed"
            value={formatNumber(player.questsComplete)}
          />
          <StatRow
            label="Tasks Completed"
            value={formatNumber(player.tasksCompleted ?? 0)}
          />
          <StatRow
            label="Market Trades"
            value={formatNumber(player.marketTrades ?? 0)}
          />
          <StatRow
            label="P2P Trades"
            value={formatNumber(player.p2pTrades ?? 0)}
          />
          <StatRow
            label="Bounties"
            value={formatNumber(player.bountiesCompleted ?? 0)}
          />
          <StatRow
            label="Chests Opened"
            value={formatNumber(player.chestsOpened ?? 0)}
          />
          <StatRow
            label="Dailies"
            value={formatNumber(player.dailiesUnlocked ?? 0)}
          />
          <StatRow label="Awards" value={formatNumber(player.awards ?? 0)} />
          <StatRow
            label="Avatars"
            value={formatNumber(player.avatarsUnlocked ?? 0)}
          />
          <StatRow
            label="Reputation"
            value={formatNumber(player.reputation ?? 0)}
          />
          {player.professionTime && (
            <StatRow label="Profession Time" value={player.professionTime} />
          )}
          {player.totalExp !== undefined && (
            <StatRow label="Total EXP" value={formatNumber(player.totalExp)} />
          )}
          {player.joinDate && (
            <StatRow label="Joined" value={player.joinDate} />
          )}
        </div>
      </div>
      ...
      <p className="text-[10px] text-muted-foreground text-center">
        Last updated: {new Date(player.lastUpdated).toLocaleTimeString()} · Mock
        data (connect API key in Settings)
      </p>
    </div>
  );
}
    ...
    <div className="game-card">
      ...
      <div className="flex gap-1 bg-secondary/50 rounded-lg p-0.5 mb-2">
        {(["daily", "weekly", "monthly"] as const).map(tab => {
          ...
          return (
            ...
            >
              {tab.charAt(0).toUpperCase() + tab.slice(1)} ({c}/
              {tabTasks.length})
            </button>
          );
        })}
      ...
      <div className="space-y-1.5">
        {filteredTasks.map((task, i) => {
          ...
          return (
            <div
              key={`task-${activeTab}-${i}`}
              className={`p-2 rounded-md ${task.isCompleted ? "bg-success/10" : "bg-secondary/20"}`}
            >
              <div className="flex items-center justify-between mb-0.5">
                <span
                  className={`text-[11px] font-medium ${task.isCompleted ? "text-success line-through" : ""}`}
                >
                  {task.description}
                </span>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-[9px] text-xp">
                  {formatNumber(task.expReward)} EXP
                </span>
                {task.otherReward && (
                  <span className="text-[9px] text-gold-accent">
                    {task.otherReward}
                  </span>
                )}
              </div>
...
function TempleBoostCard({
  ...
  temple: {
    ...
    bonus: string;
    expiresAt: number;
    worshipsUsedToday: number;
    maxWorshipsPerDay: number;
    worshipResetAt: number;
  };
}) {
  const [timeLeft, setTimeLeft] = useState(
    formatTimeRemaining(temple.expiresAt),
  );
  const [resetTime, setResetTime] = useState(
    formatTimeRemaining(temple.worshipResetAt),
  );
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-0.5 text-[9px] text-muted-foreground">
            <span>
              Worships: {temple.worshipsUsedToday}/{temple.maxWorshipsPerDay}
            </span>
