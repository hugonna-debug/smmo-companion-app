  ...
  if (bosses.length === 0) {
    return (
      <div className="p-4 flex flex-col items-center justify-center gap-3 min-h-[50vh]">
        ...
        <p className="text-[11px] text-muted-foreground/60">Connect API key to see live boss data</p>
      </div>
    );
  }
    ...
    <div className="p-3 md:p-4 space-y-3 max-w-4xl">
      ...
      <div className="space-y-2">
        {sorted.map((boss) => {
          ...
          const difficulty = levelDiff > 100 ? "Extreme" :
            levelDiff > 50 ? "Hard" : levelDiff > 0 ? "Medium" : "Easy";
          const diffColor = levelDiff > 100 ? "text-destructive" :
            levelDiff > 50 ? "text-warning" : levelDiff > 0 ? "text-chart-3" : "text-success";

          const enableDate = new Date(boss.enableTime * 1000);
          const elapsed = Date.now() - enableDate.getTime();
          const elapsedStr = elapsed < 3600000 ? `${Math.floor(elapsed / 60000)}m` :
            elapsed < 86400000 ? `${Math.floor(elapsed / 3600000)}h` :
            `${Math.floor(elapsed / 86400000)}d`;

          return (
            <div key={boss._id} className={`game-card ${isLow ? "border-destructive/30" : ""}`}>
              <div className="flex items-start gap-3">
                <div className={`size-10 rounded-lg flex items-center justify-center text-lg shrink-0 ${
                  boss.isGod ? "bg-rarity-celestial/15 border border-rarity-celestial/30" : "bg-destructive/10 border border-destructive/20"
                }`}>
                  {boss.isGod ? <Crown className="size-5 text-rarity-celestial" /> : <Skull className="size-5 text-destructive" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h2 className="text-sm font-bold truncate">{boss.name}</h2>
                    {boss.isGod && (
                      <Badge className="text-[9px] bg-rarity-celestial/15 text-rarity-celestial border-0">GOD</Badge>
                    )}
                    <span className={`text-[10px] font-medium ${diffColor}`}>{difficulty}</span>
                  </div>
                  <div className="flex items-center gap-3 mt-0.5 text-[10px] text-muted-foreground">
                    <span>Lv. {boss.level}</span>
                    <span className="flex items-center gap-0.5"><Clock className="size-2.5" />{elapsedStr} ago</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className={`text-[11px] font-mono font-bold ${hpPct > 75 ? "text-success" : hpPct > 40 ? "text-warning" : "text-destructive"}`}>
                      {hpPct}%
                    </span>
                  </div>
