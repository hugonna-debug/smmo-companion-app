...
export const fetchPvpTargets = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!apiKeys) throw new Error("SimpleMMO API key not found. Please add it in Settings.");

    // Reserve a slot in the rate limit window
    const reservedAt = await ctx.runMutation(internal.syncPlayer.internalReserveSmmoRequest, { userId });
    ...
    try {
      ...
      const { json, rate } = await smmoFetchJson(apiKeys.smmoApiKey, "/v1/pvp/targets");
      ...
      if (reservedAt !== null) {
        await ctx.runMutation(internal.syncPlayer.internalReleaseSmmoRequest, { userId, reservedAt });
      }
      throw error;
    }
  },
...
export const getRateLimit = query({
  ...
  handler: async (ctx) => {
    ...
    const requestsInLastMinute = (apiKeys.smmoRequestLog || []).filter(t => t > windowStart).length;
